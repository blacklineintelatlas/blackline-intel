...
countries = {
    ...
    'Venezuela': 'Venezuela', 'India': 'India', 'Pakistán': 'Pakistan', 'Mexico': 'Mexico', 'México': 'México'
}

# Palabras clave estratégicas actualizadas
keywords = [
    # Conflictos armados y ataques
    'attack', 'missile', 'military', 'conflict', 'explosion',
    'airstrike', 'sniper', 'ambush', 'assassination', 'bombing',
    'drone strike', 'tank assault', 'firefight', 'base attacked',

    # Política y desestabilización
    'election', 'vote', 'fraud', 'boycott', 'opposition', 'referendum',
    'diplomatic crisis', 'sanctions', 'military coup', 'golpe de estado',
    'coup', 'coup attempt', 'mercenaries', 'foreign fighters', 'cartel'

    # Seguridad internacional
    'terrorism', 'strike', 'nuclear', 'bioweapon', 'chemical attack',
    'espionage', 'cyberattack', 'hackers', 'espionage network',
    'embassy evacuation', 'diplomat killed', 'evacuation alert',

    # Términos clave geoestratégicos
    'zone of instability', 'gray zone', 'grey zone', 'military alliance',
    'proxy war', 'arms shipment', 'training mission', 'PMC',
    'private military', 'contractors', 'wagner', 'blackwater',

    # Potencias y regiones críticas
    'Putin', 'Biden', 'Xi Jinping', 'Lukashenko', 'Erdogan',
    'Iran', 'Israel', 'China', 'Taiwan', 'Hamas', 'Hezbollah',
    'Venezuela', 'Ukraine', 'Russia', 'NATO', 'BRICS', 'UN',
    'Gaza', 'West Bank', 'Kashmir', 'Nagorno-Karabakh', 'Sahel', 'mexico'

    # Crisis económicas y sociales
    'economic collapse', 'currency crisis', 'fuel shortage',
    'food crisis', 'energy crisis', 'migration wave', 'refugees',

    # España y frontera sur
    'Ceuta', 'Melilla', 'inmigrantes', 'asalto frontera', 'Guardia Civil'

    'attack', 'missile', 'military', 'conflict', 'explosion',
    'terrorism', 'strike', 'border clash', 'nuclear', 'embassy',
    'assassination', 'diplomatic crisis', 'ambush', 'sniper', 'evacuation''Trump',
...
# 🔁 Integración con GDELT mejorada
import zipfile
import io

def obtener_alertas_gdelt():
    try:
        print("📡 Obteniendo archivo GDELT más reciente...")
        url_update = "http://data.gdeltproject.org/gdeltv2/lastupdate.txt"
        nombre_archivo = requests.get(url_update).text.strip().split(" ")[2]
        url_descarga = nombre_archivo
        print(f"🔗 Descargando: {url_descarga}")
        ...
        try:
            ...
            print("⚠️ El archivo no es un ZIP real. Leyendo como CSV plano...")
            df = pd.read_csv(io.BytesIO(response.content), sep='\t', header=None)

        print(f"✅ Archivo cargado con {len(df)} filas y {df.shape[1]} columnas.")

        if df.shape[1] < 59:
            print("❌ Archivo inválido: no tiene suficientes columnas para procesar.")
            return
        ...
        df_alertas.to_excel("../data/alertas_gdelt.xlsx", index=False)
        print(f"✅ GDELT actualizado con {len(df_alertas)} alertas.")
    except Exception as e:
        print(f"❌ Error al obtener alertas de GDELT: {e}")
